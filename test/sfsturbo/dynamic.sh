#! /bin/sh
echo -e "====== Start Test SFS Turbo(dynamic) "
...
for (( i=0; i<20; i++));
do
    lines=`kubectl get pvc | grep sfsturbo-pvc-dynamic | grep Bound | wc -l`
    if [ "$lines" = "1" ]; then
        testRes="true"
        break
    else
...
for (( i=0; i<10; i++));
do
    lines=`kubectl get pod | grep sfsturbo-nginx-dynamic | grep Running | wc -l`
    if [ "$lines" = "1" ]; then
        testRes="true"
        break
    else
...
kubectl delete pod sfsturbo-nginx-dynamic
kubectl delete pvc sfsturbo-pvc-dynamic
kubectl delete sc sfsturbo-sc

if [ "$testRes" = "true" ]; then
    echo -e "------ PASS: SFS Turbo(dynamic) Test\n"
    exit 0
else
    echo -e "------ FAIL: SFS Turbo(dynamic) Test\n"
    exit 1
fi
