...
func (m *Mount) ScanForAttach(devicePath string) error {
	...
	for {
		...
		case <-timer.C:
			return fmt.Errorf("Could not find attached Cinder disk %s. "+
				"Timeout waiting for mount paths to be created.", devicePath)
		}
	}
}
...
func (m *Mount) IsLikelyNotMountPointAttach(targetpath string) (bool, error) {
	...
	klog.V(3).Infof("check mount path: %v, err: %s", notMnt, err)

	if err != nil {
		klog.V(3).Infof("check mount path, is exist: %v", os.IsNotExist(err))
		...
		klog.V(3).Infof("check mount path again, is exist: %v", os.IsNotExist(err))
	}
	return notMnt, err
}
...
func (m *Mount) UnmountPath(mountPath string) error {
	...
	return mount.CleanupMountPoint(mountPath, m.BaseMounter, false)
}
